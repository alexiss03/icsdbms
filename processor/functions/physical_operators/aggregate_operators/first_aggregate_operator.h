/*
	Performs the first aggregate operator
*/
void * first_aggregate_operator(TABLEDATALIST ** table, TABLEDATALIST * column, TABLEDATALIST * group_grouping)
{
	TABLEDATALIST 	* temp,
					* temp2,
					* start_row,
	...
	if(group_grouping == NULL)
	{
		...
		{
			append_new_column(table, "first(" + get_string(column->data) + ")", get_string(column->down->data));	
			first_op(table, column->down->down, column->up, table[0]->prev);
	...
	else
	{
		...
		if(temp->type.compare("int") == 0 || temp->type.compare("float") == 0 || temp->type.compare("varchar") == 0 )
		{
			do
			{
				...
				temp2 = end_row->down;
				first_op(table, get_cell(table, find_row_head(table, start_row), "", column),
...
/*
	Finds the actual first row given the range of rows
*/
void * first_op(TABLEDATALIST ** table, TABLEDATALIST * start, TABLEDATALIST * end, TABLEDATALIST * column)
{	
	...
	if(start->type.compare("int") == 0)
	{
		while(temp != end->down->down)
		{
			if(temp == start)
			{
				...
				create_new_cell(new int(get_int(start->data)), "int", current_row->next->down, current_row->prev->down, current_row, NULL);
			}
			else
				if(temp->up != start)
					remove_row(temp->up);
	...
	else if(start->type.compare("float") == 0)
	{
		while(temp != end->down->down)
		{
			...
			{
				current_row = find_first_null_row(column);
				create_new_cell(new float(get_float(start->data)), "float", current_row->next->down, current_row->prev->down, current_row, NULL);
			}
			else
				if(temp->up != start)
					remove_row(temp->up);
	...
	else if(start->type.compare("varchar") == 0)
	{
		while(temp != end->down->down)
		{
			...
			{
				current_row = find_first_null_row(column);
				create_new_cell(new string(get_string(start->data)), "varchar", current_row->next->down, current_row->prev->down, current_row, NULL);
			}
			else
				if(temp->up != start)
					remove_row(temp->up);
