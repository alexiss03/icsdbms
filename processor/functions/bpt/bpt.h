 ...
 */
TABLEDATALIST ** get_leaves_asc( node * root, int row_count) {
	int 			i,
					count = 0;
	...
	rows = (TABLEDATALIST **)malloc(sizeof(TABLEDATALIST *) * row_count + 1);
	...
	while (true) {
		for (i = 0; i < c->num_keys; i++) {
			...
			//*(rows + count++) = ((record *)(c->pointers[i]))->data;
			TABLEDATALIST * data = ((record *)(c->pointers[i]))->data;
...
TABLEDATALIST ** get_leaves_desc( node * root, int row_count) {
	int 			i,
					count = 0;
 ...
 */
void find_and_print(node * root, float key, bool verbose) {
	record * r = find(root, key, verbose);
	if (r == NULL)
		printf("Record not found under key %f.\n", key);
	else 
		printf("Record at %lx -- key %f, value %d.\n",
...
int find_range( node * root, int key_start, int key_end, bool verbose,int returned_keys[], void * returned_pointers[]) {
	int i, num_found=0;
	node *n;
	n = find_leaf( root, key_start, verbose );
	if (n == NULL) return 0;
 ...
 */
node * find_leaf( node * root, float key, bool verbose ) {
	int i = 0;
	node * c = root;
	if (c == NULL) {
		if (verbose) 
 ...
 * a key refers.
 */
record * find( node * root, float key, bool verbose ) {
	int i = 0;
 ...
 */
record * make_record(int value, TABLEDATALIST * data) {
 ...
 */
node * insert_into_leaf( node * leaf, float key, record * pointer ) {
 ...
 * in half.
 */
node * insert_into_leaf_after_splitting(node * root, node * leaf, float key, record * pointer) {
 ...
 * start a new tree.
 */
node * start_new_tree(float key, record * pointer) {
 ...
 * properties.
 */
node * insert( node * root, float key, int value, TABLEDATALIST * data) {
	 ...
	 * value.
	 */
	pointer = make_record(value, data);
 ...
 */
node * delete_entry( node * root, node * n, float key, void * pointer ) {
 ...
 */
node * delete_node(node * root, float key) {
